...
let places
  try {
    places = await Place.find({creator: userId}) //userWithPlaces = User.findById(userId).populate('places')
  } catch (err) {
    const error = new HttpError('Fetching places failed. Try again later',500)
    return next(error)
  }
  
 //userWithPlaces || userWithPlaces.places.length
...
const createPlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError('Creating a place failed. Try again 2ND', 500)
    return next(error)
  }
